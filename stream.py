...
class StreamManager:
    ...
    def play_stream(self, url, x, y, width, height):
        command = [
            'ffplay', '-vcodec', 'h264_v4l2m2m', '-an', '-x', str(width), '-y', str(height), '-left', str(x), '-top', str(y), '-noborder', '-loglevel', 'quiet', '-sync', 'ext', '-framerate', '12', url
        ]
        with self.lock:
    ...
    def stream_to_youtube(self, input_url, youtube_url, youtube_key):
        command = [
            'ffmpeg', '-i', input_url, 
            '-c:v', 'h264_v4l2m2m',  # Hardware-accelerated video encoding
            '-r', '12',
            '-preset', 'ultrafast', 
            '-maxrate', '2000k', 
            '-bufsize', '6000k', 
